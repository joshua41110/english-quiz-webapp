...
export default function EnglishQuizApp() {
  ...
  const questionsURL =
    "https://script.google.com/macros/s/AKfycbz7ZP8Gvtl8J4SyV4UPScvxgGwYgorDBzpdLFwxpjNOy8e3ixE-pEEoA0uKVcs4wxw/exec";
  const submitURL =
    "https://script.google.com/macros/s/AKfycbx1JbTy_bP88qkhgaG1Jv1An78qkWEeYi7CwkmGD8Gm4n_Eh-bs6yUyk48v2zzVUto/exec";
  const resultCSV =
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vRpq_qOHDW4OYLGFyj0bSqMLBRmcW_xcGgbzAr1hs8bLNZXRK3V0yPAbLmYibK9Rne7WCp6u4pOl6TX/pub?output=csv";
  ...
  const handleSubmit = async () => {
    const score = answers.reduce((acc, ans, idx) => {
      return (
        acc +
        (ans.trim().toLowerCase() === questions[idx].en.toLowerCase() ? 1 : 0)
      );
    }, 0);
  ...
  const fetchAnswers = () => {
    fetch(resultCSV)
      ...
      .then((csv) => {
        ...
        const scoreIndex = header.findIndex(
          (h) => h.trim().toLowerCase() === "score"
        );
        ...
        const parsed = latestTwo.map((row) => {
          ...
          const scoreRaw = parts[scoreIndex]?.replace(/^"|"$/g, "").trim();
        ...
        const rank = sorted.findIndex(
          (r) => r.name === playerName && r.score === playerScore
        );
        setResults(sorted);
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-8">
      ...
      {!submitted ? (
        ...
        <div className="text-center space-y-4">
          ...
          <p className="text-xl">
            你答對了 {playerScore} / {questions.length} 題
          </p>
